...
(survival_counts.tidyplot()
 .add_pie(mapping={'x': 'survived', 'y': 'count'})
...
(day_counts.tidyplot()
 .add_donut(mapping={'x': 'day', 'y': 'count'}, inner_radius=0.6)
...
(class_counts.tidyplot()
 .add_pie(mapping={'x': 'day', 'y': 'total_bill'}, fill=colors)
 .save('figures/tips_by_day_pie.png'))

# Second chart - Tips
(class_counts.tidyplot()
 .add_donut(mapping={'x': 'day', 'y': 'tip'}, inner_radius=0.7, fill=colors)
...
(cut_counts.tidyplot()
 .add_donut(mapping={'x': 'cut', 'y': 'count'}, inner_radius=0.7)
...
plot = (survival_by_class.tidyplot()
        .add_pie(mapping={'x': 'survived', 'y': 'count'})
...
(class_counts.tidyplot()
 .add_donut(mapping={'x': 'class', 'y': 'count'}, inner_radius=0.8, fill=['#FF9999', '#66B2FF', '#99FF99'])
...
(class_counts.tidyplot()
 .add_donut(mapping={'x': 'class', 'y': 'count'}, inner_radius=0.6, fill=['#FF9999', '#66B2FF', '#99FF99'])
...
(survival_counts.tidyplot()
 .add_donut(mapping={'x': 'survived', 'y': 'count'}, inner_radius=0.7, fill=['#ffcc99', '#ff99cc'])
...
for i, passenger_class in enumerate(titanic['class'].unique()):
    ...
    (class_survival.tidyplot()
     .add_donut(mapping={'x': 'survived', 'y': 'count'}, inner_radius=0.6, fill=['#ff9999', '#66b3ff'])
...
(class_counts.tidyplot()
 .add_pie(mapping={'x': 'class', 'y': 'count'},
          explode=[0.1, 0.1, 0.2],  # Third class more exploded
          start_angle=45,  # Rotate 45 degrees
          show_labels=True,
          label_type='{:.0f}',  # Custom format
          fill=['#FF9999', '#66B2FF', '#99FF99'])
...
(cut_counts.tidyplot()
 .add_donut(mapping={'x': 'cut', 'y': 'count'},
            inner_radius=0.7,
            sort=True,
            show_labels=True,
            label_type='percent',
            label_radius=0.85,  # Labels inside the donut
            label_size=8)
